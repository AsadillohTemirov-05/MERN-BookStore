...
const Cart = () => {
  ...
  books.map((item) => {
    if (cartItems[item._id] > 0) {
      return (
        <div key={item._id} className='bg-white p-2 mt-3'>
        <div className='flex gap-x-3'>
            <div className='flex items-start gap-6'>
                <img src={item.image} className='w-14 h-14 rounded' alt="" />
            </div>
...
<div className='flex mt-28'>
  <div className='w-full sm:w-[450px]' >
    <CartTotal/>
    <button onClick={()=>navigate('/place-order')} className='btn btn-secondaryOne mt-7'>
      Proceed To CheckOut
    </button>
  </div>
</div>
